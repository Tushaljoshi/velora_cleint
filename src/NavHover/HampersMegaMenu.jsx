...
import Hamper from "../assets/b2.png"
...
const hampersMenu = [
  {
    ...
    items: [
      ...
      { label: "Anniversary Hampers" },
      { label: "Premium Gift Hampers", badge: true },
      { label: "Wedding Hampers" },
      { label: "Luxe Hampers", badge: true }
    ]
  },
  {
    title: "Thoughtfully Curated Gifts",
    items: [
      { label: "All Gift Hampers" },
      { label: "Healthy Hampers" },
      { label: "Snacks Hampers" },
      { label: "Grooming Hampers" },
      { label: "Tea n Coffee Hampers" },
      { label: "Gourmet Hampers" },
      { label: "Chocolate Hampers" },
      { label: "Fruit Hampers" },
      { label: "Sweet Hampers" },
      { label: "Dry Fruit Hampers" },
      { label: "DIY Kits Hampers" }
    ]
  },
  {
    title: "Featured Gift Sets",
    sections: [
      {
        items: [
          { label: "All Gift Sets" },
          { label: "Best Sellers" },
          { label: "New Arrivals" },
          { label: "Birthday Gift Sets" },
          { label: "Anniversary Gift Sets" },
          { label: "Wedding Gift Sets" }
        ]
      },
      {
        title: "Cake Surprise Sets",
        items: [
          { label: "Cake Surprise Sets" },
  ...
  },
  {
    ...
    items: [
      { label: "All Floral Gift Sets" },
      { label: "Flowers n Cakes" },
      { label: "Flowers n Chocolates" },
      { label: "Flowers n Plants" },
      { label: "Flowers n Guitarist" },
      { label: "Birthday Flowers n Cakes" },
      { label: "Birthday Flowers n Chocolates" },
      { label: "Anniversary Flowers n Cakes" },
      { label: "Anniversary Flowers n Chocolates" }
    ]
  }
]
...
export default function HampersMegaMenu() {
  ...
  return (
    <div className="relative">
      ...
      <div className="hidden lg:flex absolute left-0 w-[1500px] bg-[#f2e5e5] shadow-xl border border-[#e6e6e6] p-6 gap-6 z-50">
            ...
            {col.sections ? (
              col.sections.map((section, j) => (
                <div key={j} className={j > 0 ? "mt-5" : ""}>
                  {section.title && (
                    <h4 className="text-[14px] font-bold text-black mb-2">{section.title}</h4>
                  )}
        ...
        <img src={Hamper} className="absolute bottom-4 right-4 h-[100px] w-[100px]" alt="" />
      </div>
      ...
      <div className="lg:hidden" style={{ background: "#f2e5e5" }}>
        ...
        {hampersMenu.map((col, i) => (
          <MobileSection key={i} col={col} navigate={navigate} />
        ))}
