...
import Plant from "../assets/b2.png"

const plantsMenu = [
  {
    title: "Elite Greens",
    items: [
      { label: "Lush Ferns Galore" }, { label: "All Plants" }, { label: "Best Sellers" },
      { label: "Same Day Delivery" }, { label: "Luxe Plants", badge: true },
      { label: "New Arrivals" }, { label: "Premium Plants" }, { label: "Air Purifying Plants" },
      { label: "Plants for Her" }, { label: "Plants for Him" },
      { label: "Kokedama Plants", badge: true }, { label: "Plants Offers" },
      { label: "Garden Accessories" }, { label: "Terrariums Plants" }, { label: "Personalised Plants" }
    ]
  },
  {
    ...
    sections: [
      {
        items: [{ label: "Indoor Plants" }, { label: "Desktop Plants" }]
      },
      {
        title: "For Celebrations",
        items: [
          { label: "Birthday" }, { label: "Anniversary" },
          { label: "House Warming" }, { label: "Good Luck" }
        ]
      }
    ]
  },
  {
    ...
    items: [
      { label: "Money Plants" }, { label: "Snake Plants" }, { label: "Jade Plants" },
      { label: "Plants DIY Kits" }, { label: "Lucky Bamboo" }, { label: "Palm Plants" },
      { label: "Bonsai Plants" }, { label: "Ficus Plants" }, { label: "Spider Plants" },
      { label: "Exotic Plants" }, { label: "Agalaonema Plants" }, { label: "Flowering Plants" },
      { label: "Cactus n Succulent Plants" }, { label: "Low Maintenance Plants" }
    ]
  },
  {
    title: "Planters Style",
    items: [
      { label: "Ceramic Planters" }, { label: "Metal Planters" }, { label: "Glass Planters" },
      { label: "Mugs Planters" }, { label: "Planter Pots" }, { label: "Cake n Plants" },
      { label: "Plants Combos" }, { label: "Flowers n Plants" }
    ]
  },
  {
    title: "Send Plants To",
    items: [
      { label: "Delhi NCR" }, { label: "Bengaluru" }, { label: "Mumbai" },
  ...
  return (
    <div className="relative">
      <div className="hidden lg:flex absolute left-0 w-[1500px] bg-[#f2e5e5] shadow-xl border border-[#e6e6e6] p-6 gap-6 z-50">
                  ...
                  <ul className="flex flex-col gap-[4px]">
                    {section.items.map((item) => (
                      <li key={item.label} className="flex items-center gap-2">
                        <a href="#" className="text-[13px] text-[#333] hover:text-[#e8003d]">{item.label}</a>
        ...
        <img src={Plant} className="absolute bottom-4 right-4 h-[100px] w-[100px]" alt="" />
      </div>
      ...
      <div className="lg:hidden" style={{ background: "#f2e5e5" }}>
        <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "10px 16px", borderBottom: "1px solid #e8d5d5", scrollbarWidth: "none" }}>
          {[
            { label: "Money Plants" }, { label: "Luxe Plants", badge: true },
            { label: "Air Purifying Plants" }, { label: "Bonsai Plants" },
            { label: "Kokedama Plants", badge: true }, { label: "Ceramic Planters" },
          ].map(item => (
        ...
        {plantsMenu.map((col, i) => <MobileSection key={i} col={col} navigate={navigate} />)}
      </div>
    </div>
  )
}